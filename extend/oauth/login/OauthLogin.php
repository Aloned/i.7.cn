...
abstract class OauthLogin
{
    ...
    public function __construct($token = null)
    {
        ...
        $this->Type = explode('\\', strtoupper(substr($class, 0, strlen($class) - 3)))[3];

        //获取应用配置
        $config = config('sdk')["LOGIN_SDK_{$this->Type}"];
    ...
    public static function getInstance($type, $token = null)
    {
        ...
        if (class_exists("oauth\login\sdk\\{$name}")) {
            $class_name = "oauth\login\sdk\\{$name}";
